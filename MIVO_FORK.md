...
- Current prerelease: `0.18.1-mivo.20`
...
### Platform interaction convention (Windows)

On Windows, the canvas follows the desktop convention used by tools such as Figma: a plain mouse wheel zooms the canvas (shift+wheel still pans horizontally, Ctrl/Cmd+wheel and trackpad pinch zoom on every platform), and dragging with the secondary (right) mouse button pans the canvas. Non-Windows platforms keep upstream Excalidraw behavior unchanged.
...
- Managed Connector background ordering is traversed directly in two render
  layers without materializing a reordered copy of all visible elements.
